        ...
        VStack(spacing: 32) {
            ...
            .onTapGesture {
                withAnimation(.spring(response: 0.5, dampingFraction: 0.7)) {
                    // Döndürme animasyonu için şova geçiş
                    showTranslation.toggle()
                }
            }
            // 3D döndürme efekti
            .rotation3DEffect(
                showTranslation ? .degrees(180) : .degrees(0),
                axis: (x: 0.0, y: 1.0, z: 0.0)
            )
            // Yazının doğru görünmesi için ters çevirme
            .modifier(FlipModifier(isFlipped: showTranslation))
            Spacer()
...
// Kart çevrildiğinde yazının doğru görünmesi için özel modifier
